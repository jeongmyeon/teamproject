...
function AdminNotificationHandler({ userRole }) {
  useEffect(() => {
    ...
    connectWebSocket((connectedClient) => {
      ...
      connectedClient.subscribe('/topic/notify/withdraw', (msg) => {
        const data = JSON.parse(msg.body);
        showStyledToast('🚫', '[탈퇴요청]', data.message);
      });

      connectedClient.subscribe('/topic/notify/competition', (msg) => {
        const data = JSON.parse(msg.body);
        showStyledToast('🏆', '[공모전]', data.message);
      });

      connectedClient.subscribe('/topic/notify/report', (msg) => {
        const data = JSON.parse(msg.body);
        showStyledToast('⚠️', '[신고]', data.message);
      });

      connectedClient.subscribe('/topic/notify/inquiry', (msg) => {
        const data = JSON.parse(msg.body);
